...
#실습2-23
select `t_a_no`,`t_dist`,sum(`t_amount`) from `bank_transaction` where `t_dist`='1' group by `t_a_no` order by `t_a_no` asc;

#실습2-24
select `t_a_no`, `t_dist`, sum(`t_amount`) as `합계` from `bank_transaction` where `t_dist`= '1' group by `t_a_no` having sum(`t_amount`) >= '100000' order by `합계` desc;
